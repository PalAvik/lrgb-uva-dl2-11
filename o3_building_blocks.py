...
class O3TensorProduct(nn.Module):
    ...
    def forward_tp_rescale_bias(self, data_in1, data_in2=None) -> torch.Tensor:
        ...
        # print("ALOHA111",data_in1.shape,data_in2.shape)

        if data_in1.shape[0]!=data_in2.shape[0]:
            # print("SUPER TYPO")
            data_in3=torch.ones_like(data_in1[:, 0:1])
            data_out = self.tp(data_in1, data_in1)
        else:
            data_out = self.tp(data_in1, data_in2)
        # print(data_out.shape,"hello")
        # Apply corrections
        if self.tp_rescale:
